...
export default function Header() {
  ...
  // ESLint error එක මඟහැරීමට සහ දත්ත ලබා ගැනීමට
  useEffect(() => {
        ...
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {user ? (
              ...
              <Link href="/signin" className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-full text-sm font-bold transition-all shadow-md">
                Join
              </Link>
            )}
          </div>
          ...
          >
            ...
            <div className="p-6">
              {user ? (
                <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-600 font-medium">Member</p>
                    </div>
                  </div>
                ...
                <Link href="/signin" onClick={() => setIsOpen(false)} className="flex items-center justify-center bg-emerald-600 text-white p-4 rounded-2xl font-bold">
                    Get Started
                </Link>
              )}
            </div>
            ...
            <div className="flex-1 px-4 flex flex-col gap-1 overflow-y-auto">
              {navLinks.map((link) => (
                <Link key={link.name} href={link.href} onClick={() => setIsOpen(false)} className="flex items-center gap-4 px-4 py-4 rounded-xl text-emerald-900 hover:bg-emerald-50 font-semibold">
